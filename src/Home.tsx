  ...
  return (
    <div className="site-shell">
      <header className={`site-nav ${scrolled ? "site-nav-scrolled" : ""}`}>
        <button className="brand-lockup" onClick={() => jumpTo("overview")} aria-label="Go to GUARDIAN home"><img className="header-logo-image header-logo-left" src={guardianLogo} alt="GUARDIAN" /></button>
        <nav className={`nav-links ${menuOpen ? "nav-links-open" : ""}`} aria-label="Primary navigation">
          <button onClick={() => jumpTo("overview")}>Overview</button><button onClick={() => jumpTo("verify")}>Verify records</button><button onClick={() => jumpTo("orgguard")}>OrgGuard</button><button onClick={() => jumpTo("about")}>About</button>
          <button className="nav-mobile-cta" onClick={() => { setMenuOpen(false); jumpTo("verify"); }}>Request access <ArrowUpRightIcon /></button>
        </nav>
        <div className="nav-actions"><button className="nav-login" onClick={() => toast("Secure sign in is coming next in the product build.")}>Sign in <CircleUserRound size={16} /></button><button className="nav-cta" onClick={() => jumpTo("verify")}>Request access <ArrowRight size={15} /></button></div>
      ...
      <main>
        <section id="overview" className="hero-section section-pad">
        ...
        <section className="logo-strip section-pad"><p>Trusted thinking for high-stakes decisions</p><div className="logo-words"><span><Building2 size={18} /> ENTERPRISE</span><span><Network size={18} /> B2B NETWORK</span><span><Globe2 size={18} /> REGIONAL DATA</span><span><Fingerprint size={18} /> LEGAL GRADE</span></div></section>

        <section id="verify" className="registry-section section-pad section-paper"><div className="section-heading-row"><div><span className="section-index">02 / REGISTRY</span><h2>Protect your <span>sustainability.</span></h2><p>Search verified records across corporate, organization and employee risk categories before you commit resources.</p></div><button className="button-outline" onClick={() => toast.success("Verification request queued for your workspace.")}>Start a private check <ArrowUpRightIcon /></button></div>
