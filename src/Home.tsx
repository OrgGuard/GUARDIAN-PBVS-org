...
type PageId = "overview" | "verify" | "orgguard" | "about";
...
function Home() {
  ...
  const [activePage, setActivePage] = useState<PageId>(() => {
    ...
    return ["overview", "verify", "orgguard", "about"].includes(hash) ? hash : "overview";
  });
    ...
    <div className={`site-shell page-${activePage}`}>
      <header className={`site-nav ${scrolled ? "site-nav-scrolled" : ""}`}>
        ...
        <nav className={`nav-links ${menuOpen ? "nav-links-open" : ""}`} aria-label="Primary navigation">
          <button onClick={() => openPage("overview")}>Overview</button><button onClick={() => openPage("verify")}>Verify records</button><button onClick={() => openPage("orgguard")}>OrgGuard</button><button onClick={() => openPage("about")}>About</button>
